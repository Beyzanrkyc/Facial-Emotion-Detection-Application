...
class Face_Emotion_CNN(nn.Module):
  ...
  def forward(self, x):
    ...
    if self.debug and not self._fc1_adjusted:
        print(f"Shape before flattening: {x.shape}")
    
    x = x.view(x.size(0), -1)

    if not self._fc1_adjusted:
        in_features = x.size(1)
        if in_features != 1024:
            print(f"Adjusting FC1 input features from 1024 to {in_features}")
            device = self.fc1.weight.device
            self.fc1 = nn.Linear(in_features, 512).to(device)
            self._fc1_adjusted = True
